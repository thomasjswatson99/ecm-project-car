...
void main(void){
    ...
    unsigned int level;
    unsigned int batterySaver = 100;

    unsigned int forwardPower = 26;
    unsigned int reversePower = 40;

    char color;
    char store[10];
    char time[10];
    signed int motions = 0;
    char complete = 0;
    char interval = 0;
    char maxTime = 23;
    ...
    struct turnDelay turnL, turnR;
    struct RGBC_val ambientR, ambientG, ambientB, ambientC;
    ...
    level = 20* ADC_getval()/21;
    sprintf(battery_level,"battery level %d%",level);
    sendStringSerial4(battery_level);
# 70 "main.c"
    if (level<20)
    {
        batterySaver = level;
    ...
    setTurnDelay(&turnL, &turnR, batterySaver);

    while (PORTFbits.RF2);
    initDCmotorsPWM(200, &motorL, &motorR);
    toggleRGB();
    scan(&ambientC,&ambientR,&ambientG,&ambientB);
    color_interrupt_clearReset();

    while (PORTFbits.RF2);


    while((motions < 10)&&(complete == 0))
    {

        fullSpeedAhead(&motorL,&motorR,forwardPower,1);
        ...
        if (LATDbits.LATD7)
        {
            ...
            if (color == 1)
            {
                ...
                color_interrupt_clearReset();
                complete = 1;
                time[motions] = interval;
                store[motions] = 0;
            }

            else
            {
                shuffle(&motorL, &motorR, 1, 0);
                time[motions] = interval;
                motions ++;



                struct turnInfo turnSpecs = turnFunction(&turnL, &turnR, color);
                turnAngle(&motorL, &motorR, turnSpecs.delay, turnSpecs.direction);
                interval = 0;
                LATDbits.LATD7 = 0;
                color_interrupt_clearReset();
            }
    ...
    while(motions>=0)
    {
        if (store[motions]!=0)
        {
            store[motions] = returnSwitch(store[motions]);
            struct turnInfo oppTurn;
            oppTurn = turnFunction(&turnL, &turnR, store[motions]);
            turnAngle(&motorL, &motorR, oppTurn.delay, oppTurn.direction);
        }

        fullSpeedAhead(&motorL,&motorR,reversePower,0);

        while(time[motions]>0)
        {
            if (LATHbits.LATH3==1)
            {
                if (time[motions]%2 == 0)
